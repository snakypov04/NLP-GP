...
def bayesian_optimization(
    ...
    # Define search space (adjusted to reduce overfitting)
    # Smaller networks with stronger regularization
    dimensions = [
        Integer(64, 256, name='hidden_units_1'),  # Reduced from 128-512
        Integer(32, 128, name='hidden_units_2'),  # Reduced from 64-256
        Integer(16, 64, name='hidden_units_3'),   # Reduced from 32-128
        Real(0.3, 0.7, name='dropout_rate'),      # Increased from 0.1-0.5
        Real(1e-5, 1e-2, prior='log-uniform',
             name='l1_reg'),  # Increased from 1e-6
        Real(1e-5, 1e-2, prior='log-uniform',
             name='l2_reg'),  # Increased from 1e-6
        Real(1e-4, 5e-3, prior='log-uniform',
             name='learning_rate'),  # Narrowed range
        Categorical(['relu', 'tanh'], name='activation')  # Removed 'elu'
    ]
    ...
    def objective(**params):
        ...
        try:
            ...
            # Use smaller batch size for better generalization
            history = model.fit(
            ...
            y_pred_mapped = np.array([index_to_label[pred]
    ...
    logger.info(
        "This may take a while. Progress will be shown for each iteration.")
    result = gp_minimize(
        func=objective,
...
def train_mlp_model(
    ...
    # Train with smaller batch size for better generalization
    history = model.fit(
        X_train, y_train_mapped,  # Use mapped labels (0, 1, 2)
        validation_data=(X_val, y_val_mapped),  # Use mapped labels
        epochs=epochs,
        batch_size=64,  # Reduced from default 128
        callbacks=[early_stopping, reduce_lr],
        class_weight=class_weights,
        verbose=1
    )
...
def plot_roc_curves(roc_curves: Dict, metrics: Dict, save_path: str) -> None:
    ...
    plt.title('ROC Curves (One-vs-Rest) - Optimized MLP',
              fontsize=16, fontweight='bold')
...
def plot_optimization_history(optimization_history: List[Dict], save_path: str) -> None:
    ...
    plt.axhline(y=max(f1_scores), color='r', linestyle='--',
                label=f'Best F1: {max(f1_scores):.4f}')
...
def compare_with_baseline(mlp_metrics: Dict, baseline_metrics_path: str) -> None:
    ...
    logger.info(
        f"  Improvement: {(mlp_metrics['accuracy'] - baseline_metrics['accuracy'])*100:.2f}%")
    ...
    logger.info(
        f"  Improvement: {(mlp_metrics['weighted_f1'] - baseline_metrics['weighted_f1'])*100:.2f}%")
...
def main():
    ...
    # Evaluate
    val_metrics, _, _, _ = evaluate_mlp_model(
        model, X_val, y_val, set_name="Validation")
